...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  def provideAuthenticatorCrypter(configuration: Configuration): Crypter = {
    new JcaCrypter(
      JcaCrypterSettings(
        configuration.underlying.getString("play.http.secret.key")
      )
    )
  }
  ...
  def provideAuthenticatorService(
    ...
    val headerName =
      configuration.underlying.getString("silhouette.authenticator.headerName")
    val issuerClaim =
      configuration.underlying.getString("silhouette.authenticator.issuerClaim")
    val authenticatorExpiry = configuration.underlying.getString(
      "silhouette.authenticator.authenticatorExpiry"
    )
    val sharedSecret = configuration.underlying.getString(
      "silhouette.authenticator.sharedSecret"
    )
    ...
    new JWTAuthenticatorService(
      JWTAuthenticatorSettings(
        ...
        authenticatorExpiry =
          Duration(authenticatorExpiry).asInstanceOf[FiniteDuration],
  ...
  def providePasswordDAO(
    userDao: UserDao,
    dbExContext: DbExecutionContext
  ): DelegableAuthInfoDAO[PasswordInfo] =
    new PasswordInfoDao(userDao)(
      ClassTag.apply(classOf[PasswordInfo]),
      dbExContext
    )
